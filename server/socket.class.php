...
class socket {
    ...
    public function __construct($host = null, $port = 0, $max = 100) {
        if ($host == null || $port == 0) {
            chdir(__DIR__);
            $config_file = file_get_contents("../config.js");
            preg_match("/serverUrl = \"(.*)\"/", $config_file, $hostarray);
            preg_match("/serverPort = \"(.*)\"/", $config_file, $portarray);
            $host = $hostarray[1];
            $port = $portarray[1];
        }
     ...
     * @param resource $client The destination socket
     * @param string $msg The message
     */
