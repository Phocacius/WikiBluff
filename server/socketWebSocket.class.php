...
class socketWebSocket extends socket {
    ...
    private function disconnected($socket) {
        ...
        $this->doActions($socket, "{\"action\": \"disconnected\"}");
    ...
    public function registerAction($class, $action, $function) {
    ...
    public function doActions($socket, $action) {

        $msg = (array)json_decode($action);
        //$this->console("INCOMING ".$action);

        $count = 0;
        foreach ($this->actions as $act) {
            if ($act["action"] == $msg["action"]) {
                call_user_func_array(array($act["class"], $act["function"]), array(
                    $msg,
                    $socket,
                    intval($socket)
                ));
        ...
        if ($count == 0) {
            echo "Keine Funktion für die Aktion '" . $msg["action"] . "' definiert!\n";
        }
